...
trainingdata_Johnson <- df_HoC_2000s %>%
  group_by(date, agenda) %>% # Group by both date and agenda
  mutate(
    context = lag(if_else(speaker != "Boris Johnson", text, ""), 
                  n = 1, default = ""),   # Previous utterance, excluding Boris Johnson
    context_2 = lag(if_else(speaker != "Boris Johnson", text, ""), 
                    n = 2, default = ""), # Second previous utterance, excluding Boris Johnson
    context_3 = lag(if_else(speaker != "Boris Johnson", text, ""), 
                    n = 3, default = ""), # Third previous utterance, excluding Boris Johnson
    combined_context = paste(context_3, context_2, context, sep = " | ")                # Combine contexts
  ) %>%
  filter(speaker == "Boris Johnson") %>% # Keep only rows where the speaker is Boris Johnson
  select(date, agenda, combined_context, text) %>% # Select relevant columns
  rename(input = combined_context, output = text) # Rename for fine-tuning format
